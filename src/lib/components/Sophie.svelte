<script lang="ts">
  ...
  let progress = $derived.by(() => {
    ...
    const distanceIntoView =
      viewportHeight - sectionElement.getBoundingClientRect().top;

    return Math.min(
      1,
      Math.max(0, distanceIntoView / sectionElement.offsetHeight),
    );
  });

  let scale = $derived(
    prefersReducedMotion.current ? 1 : 0.35 + progress * 0.65,
  );
</script>
...
>
  <div
    class="flex h-full flex-col items-center justify-end gap-2 pb-14 sm:pb-4"
  >
    <img
      src={sophie}
    ...
    />
    {#if progress > 0.75}
      <p class="rounded-full bg-white px-3 py-2 text-sm shadow-md">
